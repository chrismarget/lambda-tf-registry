	...
	for tName, tCase := range testCases {
		t.Run(tName, func(t *testing.T) {
			...
			if tCase.expErr == nil {
				...
				assert.Regexp(t, tCase.expErr, err.Error())
			}

			if tCase.expJson != "" {
				require.JSONEq(t, tCase.expJson, resp.Body)
			}
		})
	}
}
